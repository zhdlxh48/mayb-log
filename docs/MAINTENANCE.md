...
| 검색              | `src/pages/search.astro`, `search-data.astro`, `src/lib/search-text.ts`, `src/workers/search.worker.ts` |
| SEO·RSS·Sitemap   | `BaseHead.astro`, `src/lib/seo.ts`, `rss.xml.ts`, `astro.config.mjs`                                    |
| CI와 배포         | `.github/workflows/ci.yml`                                                                              |

`global.css`에는 Tailwind import, 색상·글꼴 토큰, 공통 포커스 표시만 둡니다. 화면 스타일은 해당 Astro 컴포넌트의 유틸리티 클래스로 수정합니다. Markdown과 MDX가 만든 HTML의 서식은 `ArticleProse.astro`에 모여 있습니다.

## 기능을 수정할 때

새 분류는 기존 `getContent()` 반환값과 URL helper를 먼저 사용합니다. 컴포넌트가 한 곳에서만 쓰이고 짧다면 그 페이지에 두고, 독립된 의미와 재사용이 있을 때만 분리합니다.

검색 문서는 `search-data.astro`의 `data-search-document` 요소에서 만들어집니다. `search-text.ts`가 제목·부제목·description·본문·태그·카테고리·시리즈 제목·작성자 이름을 한 문자열로 합칩니다. 본문을 읽을 때 script, style, template, noscript와 숨긴 구현용 노드는 제거하고 화면에 보이는 링크 문구와 이미지 대체 텍스트는 유지합니다. 입력이 처음 생길 때만 말뭉치와 Web Worker를 불러오며 다른 페이지에는 FlexSearch 코드가 실리지 않습니다. 첫 요청이 실패하면 같은 페이지에서 다시 입력해 재시도할 수 있습니다.

MDX에는 필요한 컴포넌트를 글에서 직접 import합니다. 공통 Provider 계층이나 범용 embed 컴포넌트를 추가하지 않습니다. 일반 HTML이나 Markdown으로 충분하면 `.md`를 유지합니다. 일반 iframe은 공통 여백·테두리·최대 너비만 적용되므로 영상 비율이 필요할 때 콘텐츠에서 `aspect-video w-full`을 지정합니다.
