...
이 문서는 로컬 구현 근거를 기록합니다. final commit, GitHub Actions, 운영 URL은 실제 배포 후 갱신합니다.
...
17. **배포:** 배포 단계 전까지 미실행.
18. **미완료:** final Git commit, GitHub Actions, Worker secret 반영, 운영 deploy/smoke test. Cloudflare D1·R2·Turnstile widget은 생성 완료. 목록 썸네일과 비밀번호 변경은 허용된 v1 제외 항목.
