...
export const upload = multer({
  ...
  limits: { files: 6, fileSize: 4 * 1024 * 1024, fields: 200 },
}).array("images", 6);
...
export function validateImages(postUuid, files = [], deleteIds = []) {
  ...
  if (deletes.some((id) => !validUuid(id)))
    throw new HttpError(400, "삭제할 이미지 경로가 올바르지 않습니다.");
  if (uploads.some((upload) => deletes.includes(upload.key.slice(-41, -5))))
    throw new HttpError(400, "같은 이미지를 업로드하고 삭제할 수 없습니다.");
...
export async function putImages(media, uploads) {
  ...
  try {
    for (const image of uploads) {
      if (await media.head(image.key))
        throw new HttpError(409, "이미지 식별자가 이미 사용 중입니다.");
      await media.put(image.key, image.body, {
        httpMetadata: {
