...
export function blogPosting(post: Post, authors: CollectionEntry<'authors'>[], image?: string) {
  return {
    ...
    headline: post.data.title, description: post.data.seo.description ?? post.data.description,
    url: absolute(`posts/${post.id}`), mainEntityOfPage: absolute(`posts/${post.id}`),
    datePublished: post.data.publishedAt.toISOString(),
    dateModified: (post.data.updatedAt ?? post.data.publishedAt).toISOString(),
    inLanguage: post.data.lang,
    author: authors.map(author => ({ '@type': 'Person', name: author.data.name, url: author.data.homepage ?? absolute(`authors/${author.id}`), sameAs: author.data.sameAs })),
    ...(image ? { image: [image] } : {}),
  };
}
