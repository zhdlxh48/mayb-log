...
async function loadContent() {
  ...
  // Astro's glob loader logs plain Markdown failures without throwing. MDX compiles separately.
  for (const entry of [...allPosts, ...pages]) {
    if (entry.filePath?.endsWith('.md') && !entry.rendered)
      throw new Error(`${entry.filePath}: Markdown 렌더링 실패. 위의 상세 오류를 수정하세요.`);
  }
  ...
  for (const post of allPosts) {
    for (const author of post.data.authors) {
      if (!authors.some((entry) => entry.id === author.id))
        throw new Error(`${post.filePath}: 없는 author: ${author.id}`);
    }
    if (post.data.series) {
      if (!allSeries.some((entry) => entry.id === post.data.series!.id))
        throw new Error(`${post.filePath}: 없는 series: ${post.data.series.id}`);
  ...
  const series = allSeries
    .map((entry) => {
      const entries = posts
        .filter((post) => post.data.series?.id === entry.id)
  ...
  return {
    ...
    tags: getTaxonomy(posts, 'tags'),
    categories: getTaxonomy(posts, 'categories'),
