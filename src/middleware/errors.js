...
export function errors(error, req, res, next) {
  ...
  const message = status >= 500 ? "요청을 처리하지 못했습니다." : inputError ? "입력값 또는 파일을 확인해 주세요." : error.message;
  res.status(status).renderPage("error", { title: `${status}`, message, noindex: true });
}
