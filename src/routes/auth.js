...
function fields(req) {
  return {
    ...
    passwordConfirmation: String(req.body.password_confirmation || ""),
  };
}
...
async function rateLimit(req, key) {
  const result = await req.app.locals
    .bindings()
    .AUTH_RATE_LIMIT?.limit({ key: `${req.ip}:${key}` });
...
authRoutes.post("/login", requireOrigin, async (req, res) => {
  ...
  await verifyTurnstile(
    req.app.locals.bindings(),
    req.body["cf-turnstile-response"],
    "login",
    req.ip,
  );
  const user = await findUserByUsername(req.app.locals.bindings().DB, input.username);
  const passwordMatches = await verifyPassword(input.password, user || FAKE_PASSWORD_RECORD);
  ...
  res.set("Set-Cookie", sessionCookie(session.rawToken));
  res.redirect(303, "/profile");
});

authRoutes.get("/signup", (req, res) =>
  res.renderPage("auth/signup", {
    title: "Sign up",
    message: String(req.query.message || ""),
    noindex: true,
    scripts: [...scripts, "/signup.js"],
  }),
);
authRoutes.post("/signup", requireOrigin, async (req, res) => {
  ...
  if (!/^[a-z0-9_]{3,32}$/.test(input.username))
    throw new HttpError(400, "아이디는 영문 소문자, 숫자, 밑줄 3~32자로 입력해 주세요.");
  if (!input.displayName || input.displayName.length > 80)
    throw new HttpError(400, "닉네임을 입력해 주세요.");
  if (input.password.length < 12 || input.password.length > 200)
    throw new HttpError(400, "비밀번호는 12자 이상 입력해 주세요.");
  ...
  res.redirect(
    303,
    "/login?message=가입이+완료되었습니다.+관리자가+D1에서+계정을+활성화한+후+로그인할+수+있습니다.",
  );
});
