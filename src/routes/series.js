...
seriesRoutes.get("/series", async (req, res) =>
  res.renderPage("series/index", {
    title: "Series",
    series: await listSeriesWithPublishedCount(req.app.locals.bindings().DB),
  }),
);
seriesRoutes.get("/series/new", loadSession, requireActive, (req, res) =>
  res.renderPage("series/new", { title: "New Series", user: req.user, noindex: true }),
);
seriesRoutes.post("/series/create", loadSession, requireActive, requireCsrf, async (req, res) => {
  ...
  if (!title || title.length > 120 || description.length > 2000)
    throw new HttpError(400, "시리즈 입력을 확인해 주세요.");
