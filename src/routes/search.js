...
import { parseSearch, searchParams, searchPosts } from "../db/search.js";
...
searchRoutes.get("/search", async (req, res) => {
  ...
  if (fragment) return res.send(req.app.locals.render("search-results", { ...result, pager, pageUrl }, true));
  res.renderPage("search", {
    title: "Search", noindex: true, filters, ...result, pager, pageUrl,
