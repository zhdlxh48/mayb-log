...
const dateInput = (epoch) =>
  epoch ? new Date(epoch * 1000 + 9 * 3600_000).toISOString().slice(0, 16) : "";
const emptyPost = () => ({
  id: null,
...
function formPost(body, authorUserId, fixedUuid) {
  ...
  if (!title || !description || !bodyMarkdown)
    throw new HttpError(400, "제목, 설명, 본문을 모두 입력해 주세요.");
  if (
    title.length > 200 ||
    String(body.subtitle || "").length > 240 ||
    description.length > 2000 ||
    bodyMarkdown.length > 500_000
  )
    throw new HttpError(400, "게시글 입력 길이를 확인해 주세요.");
  if (/!\[\s*\]\(/.test(bodyMarkdown))
    throw new HttpError(400, "이미지 대체 텍스트를 입력해 주세요.");
  if (!validUuid(uuid) || (fixedUuid && uuid !== fixedUuid))
    throw new HttpError(400, "게시글 식별자가 올바르지 않습니다.");
  ...
  if (seriesId && newSeries)
    throw new HttpError(400, "기존 시리즈와 새 시리즈를 동시에 선택할 수 없습니다.");
  ...
  if (
    ...
    newCategories.length > 30 ||
    newCategories.some((name) => name.length > 120)
  )
...
async function savePost(req, id = null) {
  ...
  const existing = id ? await findPostIdentity(bindings.DB, id) : null;
  ...
  try {
    input.post.seriesId = await findOrCreateSeries(
      bindings.DB,
      input.post.seriesId,
      input.post.newSeries,
    );
...
postRoutes.get("/posts", async (req, res) => {
  ...
  if (!page) return res.redirect(303, "/posts");
  const result = await findPublishedPosts(req.app.locals.bindings().DB, page);
  const pager = pageBlock(page, result.total);
  if (page > pager.last)
    return res.redirect(303, pager.last > 1 ? `/posts?page=${pager.last}` : "/posts");
  res.renderPage("posts/index", {
    title: "Posts",
    ...result,
    pager,
    pageUrl: (number) => (number > 1 ? `/posts?page=${number}` : "/posts"),
  });
});
...
postRoutes.get("/posts/:id/edit", loadSession, requireActive, async (req, res) => {
  ...
  res.renderPage("posts/form", {
    ...
    post,
    user: req.user,
    series: options[1].results,
    categories: options[2].results,
    images,
    noindex: true,
    scripts: [
      "/vendor/marked.js",
...
postRoutes.post("/posts", loadSession, requireActive, upload, requireCsrf, async (req, res) =>
  res.redirect(303, `/posts/${await savePost(req)}`),
);
postRoutes.post(
  "/posts/:id/update",
  loadSession,
...
postRoutes.post("/posts/:id/delete", loadSession, requireActive, requireCsrf, async (req, res) => {
  ...
  if (post)
    try {
      await deletePrefix(req.app.locals.bindings().MEDIA, post.uuid);
    } catch (error) {
      console.error(error);
    }
