...
export const load: PageServerLoad = async ({ params, url }) => {
	...
	return {
		post,
		options,
		images: await listPostImages(client, bucket, post.assetId),
...
async function save(event: RequestEvent, action: PublicationAction) {
	...
	try {
		post = await updatePost(database(), id, form.data, action);
	} catch (cause) {
...
export const actions: Actions = {
	...
	delete: async ({ params }) => {
		requireUser();
		const id = routeId(params.id);
		if (!(await deletePost(database(), id))) error(404, m.post_not_found());
		redirect(303, '/posts');
	}
};
