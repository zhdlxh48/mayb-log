  ...
  schema: ({ image }) =>
    z
      ...
      .superRefine((post, ctx) => {
        ...
        if (post.thumbnail && !post.thumbnailAlt)
          error('thumbnailAlt', 'thumbnail이 있으면 thumbnailAlt가 필요합니다.');
        if (Boolean(post.series) !== (post.seriesOrder !== undefined))
          error('seriesOrder', 'series와 seriesOrder를 함께 입력하세요.');
        if (post.updatedAt && post.updatedAt < post.publishedAt)
          error('updatedAt', 'updatedAt은 publishedAt보다 이전일 수 없습니다.');
        if (new Set(post.authors.map((author) => author.id)).size !== post.authors.length)
          error('authors', '작성자 참조가 중복되었습니다.');
      }),
});
...
const authors = defineCollection({
  ...
  schema: ({ image }) =>
    z.object({
      ...
      sameAs: z.array(z.url({ protocol: /^https?$/ })).default([]),
    }),
});
