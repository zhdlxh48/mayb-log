...
공개 로그인·회원가입 action은 `requireAuth()`로 Better Auth runtime을 확인하고, 보호 영역의 server load와 mutation은 `requireUser()`로 로그인 및 승인을 확인합니다.
...
본문은 UTF-8 기준 1MiB, 카테고리는 30개, 태그는 30개이며 태그 하나는 64글자까지 허용합니다. 검색은 검색어 200글자, 시리즈·카테고리·태그 각 20개, 태그 64글자, 작성자 아이디 30글자로 제한합니다. 일반 form의 잘못된 날짜나 상한 초과는 400으로 거절하고, Markdown Preview의 request 또는 본문 byte 상한 초과는 413으로 거절합니다.
...
DB 시각은 UTC instant로 저장하고 HTML과 JSON-LD에는 ISO 8601을 사용합니다. 화면은 현재 UI locale을 사용하며 hydration 후 브라우저 timezone으로 표시합니다. 검색 날짜 범위와 보관함의 달력 경계는 `Asia/Seoul`입니다.

## 스키마 변경과 배포

Better Auth 설정을 바꾸면 공식 CLI로 schema를 생성해 diff를 검토하고, 콘텐츠 schema는 Drizzle migration으로 관리합니다.

Post와 카테고리·태그 관계는 한 D1 batch에서 저장합니다. 태그는 별도 lookup table 없이 `post_tags(post_id, tag)`에 저장합니다. 보호 영역의 server load와 mutation action은 각각 `requireUser()`를 먼저 호출합니다.
